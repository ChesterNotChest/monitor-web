...
Server Pydantic Schema 到 TypeScript 的类型映射层。

## Purpose

定义与 monitor-server 的 `schema/http/*.py` 中所有 Pydantic 模型一一对应的 TypeScript 接口，确保前端类型系统与后端完全一致。ID 统一为 `number`，枚举统一为 server 定义的 int/str enum 值。
