...
前端运行时配置——从 Vite 环境变量组装 API 和 WebRTC URL。

## Purpose

暴露三个构建时配置入口（`VITE_SERVER_BASE_URL`、`VITE_API_PORT`、`VITE_WEBRTC_PORT`），并通过 `config.ts` 模块组装为统一的 `apiBase` 和 `webrtcBase` URL 供全应用使用。
