...
export default function WeeklyReportDetail() {
  ...
  const weekIdx = Number(weekNum) - 22;
  const week = mockWeeks[weekIdx] || mockWeeks[0];
  const title = week ? `${week.label}（${week.range}）` : `第${weekNum}周`;
  const [filters, setFilters] = useState<Record<string,string[]>>({});

  const allCameras = useMemo(()=>[...new Set(mockWeeklyReportRows.map(r=>r.camera))],[]);
  const allEvents = useMemo(()=>[...new Set(mockWeeklyReportRows.map(r=>r.event))],[]);
  const allSeverities = useMemo(()=>['高危','中危','低危'],[]);
  const allStatuses = useMemo(()=>[...new Set(mockWeeklyReportRows.map(r=>r.status))],[]);

  const filtered = useMemo(()=>mockWeeklyReportRows.filter(r=>{
  ...
  return (
    <div style={{padding:'var(--space-6)',height:'100%',overflow:'auto'}}>
      <div style={{background:'var(--bg-surface)',borderRadius:'var(--radius-md)',border:'1px solid rgba(255,255,255,.06)',overflow:'hidden'}}>
        ...
        {/* Stats bar */}
        <StatsBar data={mockWeeklyReportRows} />
          ...
          <tbody>
            {filtered.map((row,i)=>(
              <tr key={row.seq} style={{background:i%2===0?'var(--bg-canvas)':'transparent'}}>
                ...
                <td style={{...tdStyle,color:'var(--text-secondary)'}}>{row.date}</td>
...
const statDefs: ReportStat[] = [
  ...
  { label:'用户操作',value:42,icon:Activity,color:'var(--color-success-dim)',iconColor:'var(--color-success)' },
  { label:'识别结果',value:156,icon:Search,color:'var(--color-warning-dim)',iconColor:'var(--color-warning)' },
  { label:'告警处理',value:12,icon:CheckCircle,color:'var(--color-danger-dim)',iconColor:'var(--color-danger)' },
];
